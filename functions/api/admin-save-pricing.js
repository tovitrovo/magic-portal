...
export async function onRequest(context) {
  ...
  try {
    ...
    if (!id) {
      return new Response(JSON.stringify({ ok: false, error: "ID da configuração de preços ausente" }), {
    ...
    const r = await fetch(`${SB_URL}/rest/v1/pricing_config?id=eq.${encodeURIComponent(id)}`, {
      method: "PATCH",
      headers,
