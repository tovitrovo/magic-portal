...
// Avança o estágio de logística de um ou mais lotes de uma vez (ex: "marcar
// todos os pedidos pagos hoje como comprados no fornecedor"). Os rótulos e a
// ordem dos estágios vivem em shared/orderStatus.js.
export async function onRequest(context) {
  ...
  try {
    // Confirma que todos os lotes existem antes de mexer em qualquer um: um id
    // inventado no meio da lista viraria um PATCH parcial silencioso.
    const idsParam = batchIds.map(encodeURIComponent).join(",");
    const checkRes = await fetch(`${SB_URL}/rest/v1/order_batches?id=in.(${idsParam})&select=id`, { headers });
