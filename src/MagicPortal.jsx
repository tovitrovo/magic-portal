...
function AdminPage({pool,pricing:pricingProp,campaign:campProp,theme,token,nav,onReload,toast:toastFn}){
  ...
  function openShipmentWhatsAppFor(batch){
    const trackingCode=batch?.tracking_code||batch?.mandabem_rastreamento||batch?.mandabem_etiqueta||'';
    const trackingStatus=batch?.tracking_status||batch?.mandabem_status||'';
