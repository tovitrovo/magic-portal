      ...
      myOrders.map((o,i)=>{const isExp=expandedOrder===i;const st = String(o.status||'').toUpperCase();
        ...
        {isExp&&<div style={{padding:'0 14px 14px',borderTop:'1px solid rgba(255,255,255,0.04)'}}>
          ...
          {isPending&&<div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,marginTop:10}}>
              ...
              <Btn variant="ghost" onClick={async(e)=>{e.stopPropagation();try{await mpSync(o.id);toastFn('Status atualizado','success');onReloadOrders();}catch(err){toastFn('Erro: '+(err.message||String(err)),'error');}}} style={{width:'100%',fontSize:12,whiteSpace:'nowrap',justifyContent:'center'}} sfx=""><RefreshCw size={14}/> Atualizar</Btn>
              <Btn variant="warn" onClick={async(e)=>{e.stopPropagation();try{await fetch('/api/admin-mark-paid',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({batchId:String(o.id)})});toastFn('Marcado como pago','success');onReloadOrders();}catch(err){toastFn('Erro: '+(err.message||String(err)),'error');}}} style={{marginLeft:8,fontSize:12}} sfx="">Marcar pago</Btn>
              <Btn variant="danger" onClick={async(e)=>{e.stopPropagation();if(!confirm('Cancelar este pedido?')) return;try{const res=await fetch('/api/cancel-order',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({batchId:String(o.id),orderId:String(o.order_id||'')})});const j=await res.json().catch(()=>({}));if(!res.ok||!j.ok) throw new Error(j.error||'Falha ao cancelar');SFX.click();toastFn('Pedido cancelado','success');onReloadOrders();}catch(err){toastFn('Erro: '+(err.message||String(err)),'error');}}} style={{width:'100%',fontSize:12,whiteSpace:'nowrap',justifyContent:'center'}} sfx=""><X size={14}/> Cancelar</Btn>
            </div>}
