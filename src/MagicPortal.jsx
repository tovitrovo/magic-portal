  ...
  async function finalize(){
    ...
    try {
      ...
      if(!isFullBonus){
        ...
        const mpRes=await fetch(`/api/mp-create`,{method:'POST',headers:{'Content-Type':'application/json'}`,'Content-Type':'application/json'},body:JSON.stringify({orderId:String(batch.id),total:Number(total.toFixed(2)),descricao:`Pedido #${shortId} - ${totalPaid} cartas`})});
